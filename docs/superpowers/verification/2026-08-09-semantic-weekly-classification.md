...
- `qwen3.8-max` 使用 180 秒超时完整返回 51/51 条；收单主题补充业务别名后正确 49 条，准确率 96.08%，无越权、重复、多目标或结构诊断，质量门禁通过；
- 千问的 7 条低置信度结果均进入负责人复核，其中 `case-003`、`case-005` 虽与人工答案不同，但没有被静默采纳；
- `case-048`“笔笔扣业务服务号运营方案”已由中置信度错分修复为高置信度归入收单，现有模块三标注样本未因新增主题发生误归类；
- 飞书写入、消息发送和定时调度：本轮均未启用。

## 本轮规则数据发现

正式“周报板块规则表”中，线上营业厅项目组和收单项目组原有的错位样例已完成修正，并已重新生成评测 JSON。收单项目组已补充以下业务别名：

- `收支两线`、`收支两条线`、`EAST`、`FTP`、`D0`、`笔笔扣`、`易宝`、`收钱吧`、`壹当伍`、`学宝通`、`校付通`；
