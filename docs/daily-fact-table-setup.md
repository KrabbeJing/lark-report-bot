...
当前日报链路只维护一个人员汇报关系字段：`直属上级`。在本系统中它表示该成员的日报汇报人，不要求与正式组织架构中的直属领导完全一致。业务板块来自`团队名称`，敏捷周报归属来自`敏捷小组`；周报负责人、指标负责人和月报章节负责人仍在各自配置表中单独维护。
...
- 通讯录表维护成员的真实姓名、直属上级和敏捷小组。事实表中的这些快照由匹配到的通讯录记录提供。
