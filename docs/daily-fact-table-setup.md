...
联系人中的空人员字段不表示解绑指令。正常同步和 `--repair-organization` 都遵循“无依据不清空历史人员归属”：已匹配联系人没有填写`分管领导`时，已有日报事实的`分管领导`会保留，不会被自动写成空值。执行组织修复前，应先补齐通讯录中的分管领导；如确需解绑，必须由人工明确清空对应事实字段并完成确认，不能把通讯录空值当作自动删除依据。
